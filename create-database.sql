...
CREATE TABLE iist.ACCOUNT_ROLE(
	...
	CONSTRAINT ACCOUNT_ROLE_UNIQUE UNIQUE (ACCOUNT_ID, ROLE_ID)
);
...
CREATE TABLE iist.ROLE_CATEGORY(
   ...
   FOREIGN KEY(ROLE_ID) REFERENCES iist.ROLES(ROLE_ID),
   CONSTRAINT ROLE_CATEGORY_UNIQUE UNIQUE (CATEGORY_ID, ROLE_ID)
);

CREATE TABLE iist.PERMISSION(
...
CREATE TABLE iist.PERMISSION_ROLE(
	ID INT PRIMARY KEY AUTO_INCREMENT,
	ROLE_ID INT NOT NULL,
	PERMISSION_ID INT NOT NULL,
	FOREIGN KEY(PERMISSION_ID) REFERENCES iist.PERMISSION(PERMISSION_ID),
   FOREIGN KEY(ROLE_ID) REFERENCES iist.ROLES(ROLE_ID),
   CONSTRAINT PERMISSION_ROLE UNIQUE (PERMISSION_ID, ROLE_ID)
);
...
CREATE TRIGGER iist.CATEGORY_CREATE_TIMESTAMP BEFORE INSERT ON iist.CATEGORY
FOR EACH ROW
SET NEW.CREATE_TIME = NOW();

CREATE TRIGGER iist.PERMISSION_CREATE_TIMESTAMP BEFORE INSERT ON iist.PERMISSION
FOR EACH ROW
SET NEW.CREATE_TIME = NOW();
...
CREATE TRIGGER iist.CATEGORY_UPDATE_TIMESTAMP BEFORE UPDATE ON iist.CATEGORY
FOR EACH ROW
SET NEW.UPDATE_TIME = NOW();

CREATE TRIGGER iist.PERMISSION_UPDATE_TIMESTAMP BEFORE UPDATE ON iist.CATEGORY
FOR EACH ROW
SET NEW.UPDATE_TIME = NOW();
